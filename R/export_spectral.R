...
#     charges. There is no fragment-intensity model in this pipeline, so
#     every MS2 peak is written with a placeholder intensity of 100. Use
#     these libraries for m/z matching and annotation; do not interpret the
#     MS2 intensities, and do not use them for any dot-product / cosine
#     scoring that weights peaks by intensity -- with flat intensities that
#     score is a function of peak count alone.
#
# Format notes:
...
# are placeholders (see the header note).
build_ms2_library <- function(mets, dict = STANDARD_DICT,
  ...
  for (met in mets) {
    ...
    if (length(mz) == 0) next
    ord <- order(mz)
    peaks <- data.frame(mz = mz[ord], intensity = 100,
...
write_mgf <- function(records, file) {
  ...
  writeLines(c(
    ...
    "# MS2 peak intensities are placeholders (flat 100), not modelled.",
